...
routerCarrito.post("/agregar/:id", (req, res) => {
  ...
  fs.writeFile('./datosCarrito.txt', JSON.stringify(productosEnLista), error => {
    if(error){
      console.log("no se pudo guardar lista")
    }else{
      console.log("lista guardada:" + JSON.stringify(productosEnLista))
    }}
)
});
